...
/*
 ...
 - 부모 클래스의 메서드를 오버라이딩 하는 것이 목적읻.
 - 클래스를 정의하는 것이 마치 메서드를 정의하는 것과 비슷하다.
 - 마지막에 세미콜론을 반드시 기술해야한다.
 형식]
 	부모 클래스명 참조변수 = new 부모클래스명() {
 		익명클래스의 실행부;
 		부모클래스의 메서드 오버라이딩();
 	};
 	※ 부모 클래스를 상속받아 이름없는 자식클래스를 생성하는
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 : String 타입의 변수에 GrilGroup을 상속받은 익명클래스를
		 생성해서 그 참조값을 할당한다.
		 개념적으로 new GirlGroup(){};
		...
		/*
		 ...
		 따라서 아래와 같아 자식에서 확장한 멤버변수는 접근할 수 없어 에러가 발생한다.
		 접근하고 싶다면 다운캐스팅(강제형변환)을 해야하지만 익명 클래스는
		 이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
		 따라서 익명클래스에서는 멤버의 확장을 거의 사용되지 않는다.
		 대부분 오바라이딩을 목적으로 활용한다.
		 */
//		System.out.println((이름이 없으므로 형변환 불가!!)s3.mic); // 에러발생됨.
		System.out.println(s3);
		
		/*
		 오버라이딩은 부모에서 생성한 메서드를 자식에서 재정의 하는 것이므로
