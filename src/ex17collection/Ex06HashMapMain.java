...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		/*
		 인스턴스 저장 : 저장 시 기존에 저장된 동일한 Key값이 존대하면
		...
		System.out.println("name이라는 키값으로 저장된 이전의 값:"
				+ map.put("name", "홍길동"));
		/*
		 int형 즉 Integer는 String으로 형변환을 할 수 없으므로
		 타입에러가 발생한다.
		 */
		int number = 20;
//		map.put("age", number);
		 ...
		 */
		System.out.println("키값을 알 때:" + map.get("name")); // 최길동
		
		/*
		 2. 키값을 모를때(혹은 너무 많아서 하나씩 명시하기 힘들 때)
		 : keySet() 메서드를 통해 전체 Key를 Set 컬렉션으로 얻어온다.
		 해당 반환값을 확장 for문으로 반복하고, 이를 통해 Value를 인출할 수 있다.
		 */
		Set<String> keys = map.keySet();
		System.out.println("확장 for 문 적용");
		for(String key:keys) {
		...
		/*
		 객체의 존재 유무 확인
		 : Map컬렉션은 Key와 Value가 있으므로 메서드도 2개로 나눠져 있다.
		 */
		System.out.println("[객체의 존재유무 확인하기]");
