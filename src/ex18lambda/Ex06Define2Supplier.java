...
public class Ex06Define2Supplier {
	...
	public static void main(String[] args) {
		/*
		 Supplier<T>
		 : 공급자라는 의미를 가지고 있다. 매개변수는 없지만 리턴값은 있는
		 ...
		 */
		/*
		 타입매개변수로 Integer 클래스를 사용했으므로 정수값을 반환받는
		 용도로 정의하겠다는 의미이다. 0~99사이의 난수를 생성한 후
		 반환하는 람다식으로 정의되어있다.
		 */
		 ...
		 */
		System.out.println("### Iterator문으로 출력");
	...
	public static List<Integer> makeRandomNum(
		...
		// 정수를 저장하기 위한 List를 생성
//		ArrayList<Integer> li = new ArrayList<Integer>();
		/* 부모인 List로 자식인 ArrayList 참조 가능 (Ctrl + T로 상속관계 확인) */
		List<Integer> li = new ArrayList<Integer>();
		for(int i=1; i<=cnt; i++) {
			// 람다식의 get() 호출을 통해 난숫를 생성한 후 List에 추가한다.
			li.add(s.get());
		}
