...
/*
 두번째 예제에서는 멤버변수의 초기값을 모두 제거하고, 초기화를 위한 멤버 메서드를 추가한다.
 초기값이 미리 지정되어 있는 것은 인스턴스 생성 시 확장성이 떨어진다.
 */
class FruitSeller2 {
	
	int numOfApple;
	int myMoney;
	// 기존 멤버상수로 선언했던 부분을 일반 멤버 변수로 변경한다.
	int apple_price;
	/*
	  사과의 단가를 표현하는 멤버 변수가 상수로 선언되면 선언과 동시에 초기화해야한다.
	  즉, 초기값이 없는 상태로는 선언 자체가 불가능하다.
	  따라서 두번째 예제에서는기존의 상수를 일반 변수로 변경해야한다.
	 */
