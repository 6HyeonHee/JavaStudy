...
/*
 ...
 - 반환값이 없다. 다라서 선언 시 void를 사용하면 안된다.
...
public class PersonConstructor {
	...
	/*
	 개발자가 생성자를 직접 정의하지 않으면 아래와 같이 
	 매개변수와 실행부가 없는 '디폴트 생성자'가 컴파일러에 의해 자동으로 삽입된다.
	 지금까지 인스턴스 생성시 new 클래스명()과 같이 사용했던 문장이 바로
	 디폴트 생성자를 호출하는 것이었다.
	 */
//	public PersonConstructor() {}
	...
	// 생성자2 : 매개변수가 있는 형태로 정의 (1개)
	public PersonConstructor(String name) {
		/*
		 해당 this는 멤버변수와 배개변수를 구분하기 위한 용도로 사용된다.
		 클래스 자신, 즉 멤버를 가리킨다. 따라서 좌측항은 메머변수, 우측항은 매개변수가 된다.
		 */
		this.name = name;
		age = 1;
		addr = "출처불명";
	...
	public PersonConstructor(String _name, int age, String addr) {
		// 매개변수와 멤버변숨병이 다르면 this를 사용하지 않아도 된다.
		name = _name;
	...
	void initialize(String name, int age, String addr) {
		/*
		  ...
		  특성이 다르다. 따라서 멤버 메서드에서는 생성자를 호출할 수 없다.
		 */
