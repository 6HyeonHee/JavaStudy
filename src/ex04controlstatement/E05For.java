...
public class E05For {
	...
	public static void main(String[] args) {
		...
		/*
		 시나리오] for문을 이용하여 1~10까지의 정수 중 2의배수의 합을 구하는 프로그램을 작성하시오.
		 */
		...
		/*
		 ...
		 for문과 상관없이 한번만 실행한다. 즉, 잘못된 문장이다.
		 */
		for(int j=0; j<=5; j++);
		{
		...
		/*
		 ...
		 여기서 사용한 변수 j는 지역변수(Local variable)라고 한다.
		 지역변수는 해당 지역(블럭)을 벗어나면 메모리에서 소멸되는 특징을 가지고 있다.
		 */
		// for문의 블럭을 벗어나면 변수 j는 소멸되므로 에러가 발생한다.
//		System.out.println("위 for문에서 선언한 변수 =" + j);
		System.out.println("위 main함수 지역에서 선언한 변수 total="+ total);
		
		/*
		 for문의 초기값을 외부(넓은지역, 여기서는 main메소드 지역)에서 선언하면
		 해당 변수는 for문의 안쪽(좁은지역)에서 사용할 수 있다.
		 즉 아래에 선언한 변수 i는 main메소드의 지역변수로 선언된다.
		 */
		int i = 0 ;
		for(; i<=5; i++) {
		...
		for(int dan=2; dan<=9; dan++) {
			...
			for(int su=1; su<=9; su++) {
				...
				// print문은 출력은 되지만 정확한 정렬은 되지않는다.
				// => printf문에서 서식문자를 통해 칸을 미리 확보하여 정렬해서 출력한다.
