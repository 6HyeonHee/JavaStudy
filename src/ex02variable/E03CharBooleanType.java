		...
		*/
		...
		/*
		 아스키코드 : 1byte로 표현할 수 있는 문자로 영문, 숫자를 십진수로 정의한 것.
		 	ex) A => 65, a => 97로 표현된다.
		 유니코드 : 1byte로 표현할 수 없는 문자로 영문과 숫자를 제외한 모든 문자(한글, 한자등)를 
		 		  2byte로 표현한 것. / 보통 숫자가 크기 때문에 16진수로 표현한다.
		*/
		// 메모리에 65로 저장된다.
		char ch2 = 'A';
		// 정수형 변수를 선언 및 초기화 한다.
		...
		/*
		 &&(논리 And) : 엠퍼센트를 사용하고 양쪽 모두 참일 때만 참을 반환한다.
		 			   그 외에는 모두 거짓을 반환한다.
		 ||(논리 Or) : 파이프를 사용하고 둘 중 하나만 참이어도 참을 반환한다.
		 			  둘다 거짓일 때만 거짓을 반환한다.
		 */ 
		// T && F => False를 반환
		boolean bn3 = bn1 && bn2;
