...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 Java에서는 정수와 정수의 연산결과가 정수로 반환된다.
		 만약 실수로 결과를 반환받고 싶다면 형변환(Type Casting)을 해야한다.
		 */
		result1 = int1 / int2;
		System.out.println("int1 / int2 = "+ result1);
		
		/*
		 - 동일한 타입의 변수를 2개 이상 선언할 때는 컴마를 사용할 수 있다.
		 - int형보다 작은 자료형을 연산하면 CPU는 int형으로 변환하여 계산한다.
		 - CPU는 정수의 연산인 경우 int형에 최적화 되어있기 때문이다.
		 - byte, short형은 게임캐릭터의 움직이나 음원파일 등을 표현할 때 주로 사용된다.
		 */
		...
		/*
		 ...
		 아래 정수를 메모리에 로드하는 순간 발생한다.
		 이떄, 접미사(L 혹은 l)을 붙여서 int형 자료가 아닌 long형의 자료임을 컴파일러가 알려줘야함.
		 이와 같이 할당을 위해 입력되는 숫자도 자료형을 기반으로 로드되는데, 이를 가르켜 리터럴(literal)이라고 함.
		 */
		//long long3 = 2200000000; // 어러발생
