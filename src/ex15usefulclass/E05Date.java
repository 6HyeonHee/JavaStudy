...
public class E05Date {
	/*
	 Calendar 클래스를 통해 얻은 인스턴스를 초로 변환한 후 매개변수로 전달된
	 날짜 서식으로 변경한 String을 반환한다.
	 */
	
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		 getTime() : 서버의 표준시인 1970-01-01 부터 현재까지의 시간을
	...
	public static void main(String[] args) {
		...
		/*
		 Calendar 클래스는 대표적인 싱글톤 클래스 이므로 new로 인스턴스를
...
//			System.out.println(formatToday + " " + formatFuture );
			...
			/*
			 getTime()으로 얻어오는 시간은 13자리인 정수값인데 10자리까지는 순소한 초(Sec)이고,
			 11!13자리는 밀리세컨즈를 의미한다. 즉, 100으로 나눠서 뒤의 3자리를 제거하면 순수한 '초'가 된다.
