...
/*
 ...
 오버로딩 : 메서드명은 같지만 매개변수의 갯수, 타입, 순서가 다른 경우.
 		즉, 서로 다른 메서드이므로 상속받은 하위클래스에서는 
 		오버로딩 된 모든 메서드가 포함된다.
 오버라이딩 : 메서드명, 매개변수, 반환타입까지 모두 동일한 메서드를 상속관계에
 		있는 하위클래스에서 재정의한 것.
...
class C extends B {
	/*
	 ...
	 멤버메서드와 반환타입, 메서드명, 매개변수의 갯수 및 타입이
	 모두 동일하다. 즉 동일한 메서드이다.
	 */
	@Override
	public void rideMethod() {
	...
	/*
	 loadMethod()는 오버로딩된 메서드로 상위클래스에 정의된 멤버메서드와
...
public class E07RideAndLoad01 {
	public static void main(String[] args) {
		...
		ref3.loadMethod(); //A
		ref3.loadMethod(1); //B
		ref3.loadMethod(1.1); //C
		/*
		 B타입으로는 접근범위가 B클래스까지 제한되므로 
...
//		ref2.loadMethod(1.1);
		...
		/*
		 A타입으로는 접근범위가 A클래스만으로 제한되므로
		 B와 C에 정의된 메서드는 호출할 수 없다.
		 */
