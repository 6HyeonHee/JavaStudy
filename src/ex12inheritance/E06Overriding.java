...
/*
 오버라이딩(Overriding)
 : 클래스가 상속관계에 있을 때 하위클래스에서 상위클래스의 멤버머서드와 완전히
 동일한 모양으로 메서드를 재정의 하는 것을 말한다.
 오버로딩과 다른 점은 매개변수의 갯수 및 타입, 반환타입 등이 완전히 똑같은
 형태로 정의된다는 것이다.
 하위클래스에서 오버라이딩한 메서드는 상위클래서의 메서드를 가리게 되므로
...
class BaseSpeaker extends Speaker {
	...
	public void showState() {
		/*
		 ...
		 특히 오버라이딩 된 메서드를 호출할 때 super가 없으면 재귀 호출이
		 되므로 주의해야한다.
		 */
		super.showState();
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 -> 윈도우 탐색기에서 같은 이름의 파일을 붙여넣기 할 때 덮어쓸지
		...
		/*
		 ...
		 범위가 부모인스턴스까지로 제한되므로, 자식쪽에서 정의된 메서드는
		 호출할 수 없다.
		 */
