...
//	public Computer() {}
	...
	/*
	 private 접근 지정자로 선언된 멤버메서드는 외부클래스 혹은 상속받은
	 클래스에서 접근이 가능하므로, Public으로 선언된 메서드를 통해
	 간접적으로 호출해야한다.
	 */
	private void keyboardTouch() {
		System.out.println("키보트를 통해서 입력한");
	}
...
class NotebookComputer extends Computer {
	...
	public NotebookComputer(String name, int initCharge) {
		/*
		 ...
		 또한 여기서는 부모클래스에 디폴트생성자가 없는 상태이므로 생략하면
		 에러가 발생한다.
		 */
		super(name);
		battary = initCharge;
	...
	// 휴대하면서 노트북을 사용한는 것을 표현함.
	public void movigCal() {
		// 사용 전 배터리 잔량을 확인 후 사용여부 판단
		if(battary < 1) {
	...
	public void write(String penInfo) {
		...
		/*
		 ...
		 A.compareTo(B) : 문자열 A와 B를 비교하여 0이 반환되면 동일한 문자열,
		 	-1 혹은 1이 반환됨녀 다른 문자열로 판단하는 String 클래스에 정의된 메서드
		 */
		...
		/*
		 ...
		 이와 같이 부모의 멤버메서드를 호출하여 자신의 멤버처럼 사용할 수 있다.
