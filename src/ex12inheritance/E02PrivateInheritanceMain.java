...
class Account {
	/*
	 ...
	 내부에서 접근할 수 있다. 즉 상속받은 하위클래스에서는 직접 접근이
	 불가능하다.
	 */
	private int money;
	
	/*
	 이와 같이 개발자가 생성자를 직접 정의하면 디폴트생성자는 추가되지
	...
	/*
	 현재는 부모와 자식 클래스가 동일 패키지에 정의되어 있으므로
	 접근지정자가 default라도 접근은 허용된다.
...
class SavingAccount extends Account {
	/*
	 ...
	 현재 부모클래스에는 매개변수가 하나인 생성자가 유일하므로 만약 super(x)를 
	 생략하면 에러가 발생한다. 
	 */
	...
	public SavingAccount(int initVal) {
		...
		/*
		 ...
		 문장이 자동으로 삽입되는데 부모 클래스에는 이런 생성자가 정의되지
		 않았으므로 에러가 발생하는 것이다.
		 */
	}
