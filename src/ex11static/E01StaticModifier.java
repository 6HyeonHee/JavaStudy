...
/*
 ...
 - main()메서드가 실행되기 전에 초기화되므로 main() 메서드 실행 시 별도의
   인스턴스 생성 없이 변수를 사용할 수 있다.
 ...
 - 메서드 앞에 static을 붙이면 정적메서드가 된다. 정적변수와 모든 특성 동일함.
 */
class MyStatic {
	/*
	...
	/*
	 인스턴스형 멤버메서드 : 인스턴스형 메서드에서는 모든 멤버를 사용할 수 있다.
	 	인스턴스형멤버, 정적멤버 둘 다 접근 가능하다.
	 */
	void instanceMethod() {
		System.out.println("instanceVar="+ instanceVar);
		System.out.println("staticaaVar="+ staticVar);
		staticMethod();
	}
	...
	/*
	 정적 멤버메서드 : 정적 메서드 내부에서는 인스턴스형 멤버에는 접근할 수 없다.
	 		정적 멤법만 접근할 수 있다.
	 		(※ main 메서드에서는 static으로 선언된 메서드만 호출 가능함을 기억하자.)	
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		/*
		 일반적인 멤버 변수에 접근하는 방법 
		 : MyStatic 클래스의 인스턴스를 생성한 후 참조 변수를 통해 접근한다.
		 */
		...
		System.out.println("ms1.instaceVar="+ ms1.instanceVar); // 100
		System.out.println("mas2.instanceVar="+  ms2.instanceVar); // 1000
...
//		ms2.staticVar = 900;
		/*
		 따라서 위의 문장은 권장되지 않는다
		 코드레벨에 보면 서로 다른 변수에 값을 할당하는 것으로 보이기 때문.
		 아래 문장을 권장한다. 동일한 메모리에 값을 순차적으로 할당하는 것으로 보이기 때문.
		 */
		MyStatic.staticVar = 800;
		MyStatic.staticVar = 900;
		System.out.println("ms1.staticVar="+ ms1.staticVar); //900
		System.out.println("mas2.staticVar="+  ms2.staticVar); //900
...
//		MyStatic.instanceMethod(); //어러발생
		MyStatic.staticMethod(); // 권장사항 (클래스명을 통해서 정적메서드 호출)
		/*
