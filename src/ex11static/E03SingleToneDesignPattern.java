...
/*
 싱글톤 디자인패턴
 : 클래스를 설계하는 디자인 패턴의 하나로써 하나의 인스턴스 즉 하나의 메모리를
...
class SingleTone {
	...
	/*
	 ...
	 프로그램이 시작될 때 미리 Method영역에 로드되어 사용할 준비를 마치게된다.
	 */
	private static SingleTone single = new SingleTone();
	
	/*
	 생성자를 private으로 선언하면 외부 접근이 안되므로 new를 통해
	 인스턴스를 생성할 수 없다. 하지만 클래스 내부엥서는 호출할 수 있다.
	 */
	private SingleTone() {
